...
def setup_train(args):
    ...
    ckpt_root = create_experiment_export_folder(args)
    export_experiments_config_as_json(args, ckpt_root)
...
def save_test_result(ckpt_root, result):
    filepath = Path(ckpt_root).joinpath('test_result.txt')
...
def load_pretrained_weights(model, path):
    ...
    print("weights from %s loaded!" % path)
    return model
