...
class AUGSASRecDataloader(object):
    def __init__(self, args, dataset):
        ...
        self.train_triplets = dataset.train_triplets
    ...
    def _get_train_loader(self):
        dataset = AUGSASRecTrainDataset(self.train, self.max_len, 
                    self.CLOZE_MASK_TOKEN, self.num_items, self.rng, self.PAD_TOKEN, self.PAD_QUERY, self.train_triplets, self.args)
    ...
    def _get_eval_loader(self, mode):
        ...
        dataset = AUGSASRecEvalDataset(self.train, self.val, self.test, self.max_len, self.CLOZE_MASK_TOKEN, self.PAD_TOKEN, self.PAD_QUERY,
...
class AUGSASRecTrainDataset(data_utils.Dataset):
    def __init__(self, u2seq, max_len, mask_token, num_items, rng, pad_token, pad_query, triplets, args):
        self.u2seq = u2seq
        ...
        self.args = args
        self.item_pool = list(self.triplets['item2query'].keys())
        self.query_pool = list(self.triplets['query2item'].keys())
    ...
    def _pad_to_len_aug(self, seq, pad, query_seq):
        res = []
        for i, (s, q) in enumerate(zip(seq, query_seq)):
            res_ = [pad] * BOW_LEN
            # replace
            if self.rng.uniform(0,1) < self.args.item_input_replace_prob and s[0] < self.num_items:
                key = str(tuple([q_+self.num_items for q_ in q]))
                if self.rng.uniform(0,1) < self.args.graph_based_prob:
                    s = eval(self.rng.choice(self.triplets['query2item'][key]))
                else:
                    s = eval(self.rng.choice(self.item_pool))
            elif self.rng.uniform(0,1) < self.args.query_input_replace_prob and s[0] > self.num_items and len(seq) > i+1:
                key = str(tuple(seq[i+1]))
                if self.rng.uniform(0,1) < self.args.graph_based_prob:
                    s = eval(self.rng.choice(self.triplets['item2query'][key]))
                else:
                    s = eval(self.rng.choice(self.query_pool))
    ...
    def __getitem__(self, index):
        ...
        query_seq = self.u2seq[user]['query'][-length-1:]
        query_seq, query_label = query_seq[:-1], query_seq[1:]
        query = self._pad_to_len(query_seq, pad=self.pad_query)

        temp = []
        for i, (s, q) in enumerate(zip(labels, query_label)):
            if self.rng.uniform(0,1) < self.args.item_label_replace_prob and s < self.num_items:
                key = str(tuple([q_+self.num_items for q_ in q]))
                if self.rng.uniform(0,1) < self.args.graph_based_prob:
                    s = eval(self.rng.choice(self.triplets['query2item'][key]))[0]
                else:
                    s = eval(self.rng.choice(self.item_pool))[0]
            temp.append(s)
        ...
        tokens = np.array(self._pad_to_len_aug(tokens, pad=self.pad_token, query_seq=query_seq) + [[self.pad_token] * BOW_LEN] * padding_len)
...
class AUGSASRecEvalDataset(data_utils.Dataset):
