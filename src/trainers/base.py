...
class AbstractTrainer(metaclass=ABCMeta):
    def __init__(self, args, model, train_loader, val_loader, test_loader, ckpt_root, user2seq, embed_only=False):
        ...
        self.model = model.large_embed_to(self.device)
    ...
    def validate_test(self, epoch, accum_iter, mode="val", target=None):
        ...
        with torch.no_grad():
            ...
            for batch_idx, batch in enumerate(tqdm_dataloader):
                ...
                description = description.replace('NDCG', 'N').replace('Recall', 'HR')
    ...
    def _create_optimizer(self, embed_only=False):
        ...
        no_decay = ["bias", "norm"]
        if 'bpr' in args.model_code:
            no_decay = ["user_emb", "item_emb"]
        ...
        if args.optimizer.lower() == 'adagrad':
            ...
            return optim.Adagrad(optimizer_grouped_parameters, lr=args.lr)
            # return optim.Adam(self.model.parameters(), lr=args.lr, weight_decay=args.weight_decay)

        elif args.optimizer.lower() == 'sgd':
            return optim.SGD(optimizer_grouped_parameters, lr=args.lr, weight_decay=args.weight_decay)
